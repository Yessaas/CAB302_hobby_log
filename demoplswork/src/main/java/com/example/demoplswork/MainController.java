    ...
    private ListCell<Contact> renderCell(ListView<Contact> contactListView) {
        return new ListCell<>() {
            ...
            protected void updateItem(Contact contact, boolean empty) {
                ...
                // If the cell is empty, clear the text and remove the event handler
                if (empty || contact == null || contact.getFullName() == null) {
                    setText(null);
                    setOnMouseClicked(null);  // Remove the event handler when the cell is empty
                } else {
                    setText(contact.getFullName());
                    setOnMouseClicked(this::onContactSelected);  // Set the event handler when the cell has valid data
                }
            }
        };
    }
